...
class DDIMSampler_mp(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            if i < 6:
                ...
                outs = self.p_sample_ddim_interpolate(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if isinstance(c, torch.Tensor):
                ...
                ### combining different conditioning using classifier-free guidance

                c1 = c.copy()
                c1['c_crossattn'] = [c['c_crossattn'][0][0].unsqueeze(0)]
                
                c3 = c.copy()
                c3['c_crossattn'] = [c['c_crossattn'][0][2].unsqueeze(0)]

                e_t = self.model.apply_model(x, t, c3, **kwargs)
                #e_t = self.model.apply_model(x, t, c1, **kwargs)
                e_t_uncond = self.model.apply_model(x, t, unconditional_conditioning, **kwargs)
    ...
    def p_sample_ddim_interpolate(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      uc_type=None, conditional_guidance_scale_temporal=None, **kwargs):
        b, *_, device = *x.shape, x.device
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if isinstance(c, torch.Tensor):
                ...
                c2 = c.copy()
                c2['c_crossattn'] = [c['c_crossattn'][0][1].unsqueeze(0)]

                c3 = c.copy()
                c3['c_crossattn'] = [c['c_crossattn'][0][2].unsqueeze(0)]

                ###
                e_t1 = self.model.apply_model(x, t, c1, **kwargs)
                e_t2 = self.model.apply_model(x, t, c2, **kwargs)
                e_t3 = self.model.apply_model(x, t, c3, **kwargs)
                e_t_uncond = self.model.apply_model(x, t, unconditional_conditioning, **kwargs)
                            
                ### frame-wise classifier-free like guidance
                e_t1 = rearrange(e_t1, 'b c t h w -> (b t) c h w')
                e_t2 = rearrange(e_t2, 'b c t h w -> (b t) c h w')
                e_t3 = rearrange(e_t3, 'b c t h w -> (b t) c h w')
                delta_e = (e_t1 - e_t2)
                weight = torch.linspace(-1, 1, e_t1.shape[0])
                e_t = torch.zeros_like(e_t3)
                
                for i, w in enumerate(weight):
                    e_t[i] = e_t3[i] + w * delta_e[i]/delta_e[i].norm()
                #for i, w in enumerate(weight):
                    #e_t[i] = e_t1[i] + w * delta_e[i]
                e_t = rearrange(e_t, '(b t) c h w -> b c t h w', b=b)
