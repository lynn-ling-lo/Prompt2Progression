...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        ...
        ## repeat t times if neccessary
        ## reshape for context [(b t) 77 768] & time embedding
        
        if len(context.shape) == 3 and context.shape[0] == 1:
            context = context.repeat_interleave(repeats=t, dim=0)
        elif len(context.shape) == 4 and context.shape[1]==16:
            context = context.view(-1, context.shape[2], context.shape[3])
        elif len(context.shape) == 3 and context.shape[0]==16:
            ###
            context = context.view(-1, context.shape[1], context.shape[2])
        else:
            print('wrong context shape: ',context.shape)
            assert 0
    ...
    ### testing for mp feature interpolation
    '''def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
        ...
        if len(context.shape) < 4 and context.shape[0]==2:
            context_1 = context[0].unsqueeze(0).repeat_interleave(repeats=t, dim=0)
            context_2 = context[1].unsqueeze(0).repeat_interleave(repeats=t, dim=0)
            
            
        elif len(context.shape) < 4:
            context_1 = context.repeat_interleave(repeats=t, dim=0)
            context_2 = context.repeat_interleave(repeats=t, dim=0)
        else:
            context = context.view(-1, context.shape[2], context.shape[3])
        ...
        hs = []
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context_1, batch_size=b)
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context_1, batch_size=b)
            ## plug-in adapter features
            if ((id+1)%3 == 0) and features_adapter is not None:
                h = h + features_adapter[adapter_idx]
                adapter_idx += 1
        ...
        h = self.middle_block(h, emb, context=context_1, batch_size=b)
        ...
        # reshape back to (b c t h w)
        y_1 = rearrange(y_1, '(b t) c h w -> b c t h w', b=b)
        ...
        hs = []
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=context_2, batch_size=b)
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context_2, batch_size=b)
            ## plug-in adapter features
            if ((id+1)%3 == 0) and features_adapter is not None:
                h = h + features_adapter[adapter_idx]
                adapter_idx += 1
        ...
        h = self.middle_block(h, emb, context=context_2, batch_size=b)
        ...
        # reshape back to (b c t h w)
        y_2 = rearrange(y_2, '(b t) c h w -> b c t h w', b=b)
