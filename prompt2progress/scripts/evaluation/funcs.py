...
from lvdm.models.samplers.ddim import DDIMSampler, DDIMSampler_mp
...
def batch_ddim_sampling_mixp(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
                        cfg_scale=1.0, temporal_cfg_scale=None, **kwargs):
    ...
    num_frames = noise_shape[2]
    len1 = int(num_frames* 2 // 8)
    len2 = num_frames - len1 * 2
    ...
    ### mixing with spherical linear interpolation
    '''len1 = int(16 * 3 // 8)
    len2 = 16 - len1 * 2

    def slerp(val, low, high):
        omega = torch.acos(torch.clamp(torch.dot(low.flatten() / torch.norm(low), high.flatten() / torch.norm(high)), -1, 1))
        so = torch.sin(omega)
        if so == 0:
    ...
    for i in range(len2):
        t = i / (len2 - 1)
        cond_list1.append(slerp(t, conditioning[0], conditioning[1]).unsqueeze(0).unsqueeze(0))
    ...
    ### averagly mixing
    '''
    cond_mix = (conditioning[[0]]+conditioning[[1]]) / 2
    cond1 = cond_mix.unsqueeze(1).repeat(1, 16, 1, 1)

    cond['c_crossattn'] = [cond1]
    '''
    
    ### averaging mixing with attribute interpolation
    '''len1 = int(16* 3 // 8)
    len2 = 16 - len1 * 2
    
    word1 = conditioning[0][2].clone()
    word2 = conditioning[1][2].clone()
    

    other = (conditioning[0].clone()+conditioning[1].clone())/2
    conditioning[0] = other
    conditioning[1] = other
    conditioning[1][2] = word2
    conditioning[0][2] = word1

    del word1, word2, other
    cond_diff1 = (conditioning[[1]] - conditioning[[0]]) / (len2 - 1)
    cond_list1 = []
    for i in range(len2):
        cond_list1.append((conditioning[[0]] + cond_diff1 * i).unsqueeze(0))
    ...
    ### averaging mixing with attribute and noun interpolation
    '''len1 = int(16* 3 // 8)
    len2 = 16 - len1 * 2
    
    word1 = conditioning[0][2:4].clone()
    word2 = conditioning[1][2:4].clone()

    other = (conditioning[0].clone()+conditioning[1].clone())/2
    conditioning[0] = other
    conditioning[1] = other
    conditioning[1][2:4] = word2
    conditioning[0][2:4] = word1
